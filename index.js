...
app.post('/user/login', body('username').isEmail(), body('password').isLength({min:8}),function (req, res) {
  //perform basic validation
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ statusCode :403, message: "Invalid credentials" });
  }
...
io.on('connection', (socket) => {
  ...
  socket.on(config.createVariant, (request) => {
    ...
    .then((item) => { socket.emit('callback_createVariant', {
      statusCode: 200,
  ...
  socket.on(config.updateDeviceStatus, (request) => {
    ...
    updateDevice(request).then(
      function(value) { 
        ...
        socket.emit(`callback_deviceStatusUpdate:${request.variant_id}`, {
          statusCode: 200,
          message :'Update successfull',
...
async function updateVariant(request) {

}

async function updateVariantParent(request) {
...
async function updateDeviceName(request) {
